...
@Devil.on(events.NewMessage(incoming=True, from_users=AUTH, pattern='/cancel'))
...
@Devil.on(events.NewMessage(incoming=True, from_users=AUTH, pattern='/batch'))
async def _batch(event):
    ...
    async with Devil.conversation(event.chat_id) as conv: 
        if s != True:
            ...
            try:
                ...
                if value > 300:
                    await conv.send_message("You can only get upto 300 files in a single batch.")
                    return conv.cancel()
            except ValueError:
